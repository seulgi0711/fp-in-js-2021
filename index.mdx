...
전에 말씀드린대로 함수형 프로그래밍에 대한 내용을 좀 소개해드리도록 할게요.

다들 한번쯤은 함수형 프로그래밍에 대해서 들어보셨을 텐데요 함수형 프로그래밍 검색해서 이런 저런 글 읽다보면 보이는 개념들이 몇가지가 있습니다.

뭐 순수 함수, 사이드 이펙트가 없다, 그리고 펑터 모나드 같은 생소한 단어도 있었을 텐데요

이런 개념들을 하나씩 단계별로 따라가면서 어떤 것들인지 그리고 왜 필요한지 한번 같이 살펴보는 시간을 갖도록 하겠습니다.
...
먼저 함수형 프로그래밍의 코드가 어떻게 생겼는지 볼거구요.

두 번째로 함수 합성에 대한 내용을 보고

함수 합성시에 필요한 포인트프리 스타일과 커링에 대해서 보겠습니다.

마지막으로 펑터의 개념과 펑터를 이용한 에러 핸들링에 대해서 살펴보겠습니다.
...
먼저 함수형 프로그래밍으로 작성한 코드가 어떤 식으로 생겼는지 살펴보도록 하겠습니다.
...
```jsx 5:22 file=./src/CodeStyle/fp-com.js
```
...
```jsx 5:23 file=./src/CodeStyle/imper-com.js subtitle="주니어"
```

```jsx 5:22 file=./src/CodeStyle/fp-com.js subtitle="현재"
```
...
```jsx 5:23 file=./src/CodeStyle/imper-com.js subtitle="Imerative"
```

```jsx 5:22 file=./src/CodeStyle/fp-com.js subtitle="Functional"
```
...
자 여기 어떤 동작을 하는 코드가 있습니다. 왼쪽 오른쪽 모두 같은 동작을 하는 코드입니다.

그리고 이 두가지 코드는 같은 사람이 짠 코드입니다. [enter]

왼쪽은 주니어 시절 제가 짠 코드이고, 오른쪽은 현재의 제가 짠 코드입니다.

코드를 봤을때 어떤 느낌이 드시나요?

보는 사람에 따라서 다르겠지만 왼쪽은 상대적으로 코드를 읽기가 어렵고, 오른쪽은 조금더 읽기 쉬운것 같습니다.

또 뭐 가독성 외에도 여러가지 차이점이 있을텐데요. [enter]

사실 이 두 코드는 각 각 절차지향 스타일과 함수형 스타일로 작성한 코드입니다.

이 두가지 스타일의 코드가 어떻게 다른지 조금더 자세하게 살펴보도록 할게요.
...
```json file=./src/CodeStyle/name-01.json title="요구 사항" subtitle="FP 스터디 신청자 이름 목록"
```
...
여기 어떤 이름 목록이 있습니다. 그런데 입력한 형태가 통일성이 없습니다.
...
그래서 이 이름 목록을 요구사항에 맞게 변경하려고 합니다. [enter]

첫 번째로 성과 이름은 공백으로 구분해줍니다. 그러면 오른쪽 이름 목록에서 대시와 언더바로 구분된 이름이 공백으로 바뀌어야 겠죠. [enter]

그리고 두 번째로 성과 이름은 대문자로 시작하도록 작업을 할 겁니다. 
...
<Step title="Imperative" subtitle="이름 목록 순회">
...
<Step title="Imperative" subtitle="공백으로 구분: 언더바(_), 대시(-) 문자를 빈 문자열로 대체">
...
<Step title="Imperative" subtitle="대문자로 시작: 공백을 기준으로 이름을 쪼개준다.">
...
<Step title="Imperative" subtitle="대문자로 시작: 쪼개준 이름의 첫 글자를 대문자로 바꿔준다.">
...
<Step title="Imperative" subtitle="대문자로 시작: splitName을 공백을 넣어서 join 한다.">
...
<Step title="Imperative" subtitle="오름차순 정렬: sort 메소드 이용">
...
이름이 어레이기 때문에 for loop를 가지고 순회하면서 각 이름에 접근할 수 있습니다. [enter]

그리고 성과 이름을 공백으로 구분하기 위해서 replace함수를 사용합니다.

언더바와 대시를 공백으로 바꿔준 후 원래 자리에 다시 저장해줍니다. [enter]

그리고 두 번째로 성과 이름이 대문자로 시작하도록 만들기 위해서 성과 이름을 공백을 기준으로 나눠줍니다. 
...
<Step title="Functional" subtitle="Functional: map 메소드를 이용하면 순회하며 변경 가능">
...
<Step title="Functional" subtitle="공백으로 구분: replaceSpace라는 함수를 정의해서 map 함수로 넘겨준다.">
...
<Step title="Functional" subtitle="대문자로 시작: 우선 string의 첫 글자를 대문자로 바꿔주는 함수 정의">
...
<Step title="Functional" subtitle="대문자로 시작: 이름을 받아서 공백 기준으로 첫 글자만 대문자로 바꿔주는 함수를 정의">
...
<Step title="Functional" subtitle="대문자로 시작: map + changePartStartCase">
...
<Step title="Functional" subtitle="오름차순 정렬">
...
제가 함수형 약을 팔기위해서 말씀드리고는 있지만
함수형 스타일이 조금더 간결해보이고, 그러다보니 읽기도 편한것 같습니다.

그리고 함수 단위로 쪼개져있어서 테스트 코드 작성도 더 쉬울것 같습니다.
...
그렇다면 이 함수형 프로그래밍이 어떤 특징을 갖는지 살펴보도록 하겠습니다.
...
<Notes>
일급함수라는 말은 한마디로 함수를 변수처럼 사용할 수 있다. 는 뜻입니다.
</Notes>
...
```js title="일급함수(First Class Function)" subtitle="1. 변수에 할당"
...
```js 1[64:90],2,3[1] title="일급함수(First Class Function)" subtitle="2. 함수를 파라미터로 전달"
document.querySelector('.my-button').addEventListener('click', (event) => {
  console.log('버튼 클릭!');
})
```

```js 2:4 title="일급함수(First Class Function)" subtitle="3. 함수를 리턴하는 함수"
...
<Notes>
함수형 프로그래밍의 두 번째 특징으로 순수 함수가 있습니다.
</Notes>
...
```js title="순수함수(Pure Function)" subtitle="파라미터에만 의존하는 순수 함수"
const negate = (num) => {
  return num * -1;
}
```

```js title="순수함수(Pure Function)" subtitle="외부 변수에 의존하는 불순 함수(Impure Function)"
let num = 10;
...
```js title="순수함수(Pure Function)" subtitle="maxNum이 다른곳에서 바뀐다면?"
let maxNum = 5;
...
```js title="순수함수(Pure Function)" subtitle="불순 함수: 사이드 이펙트가 발생할 가능성이 있다."
const isGraterThanFive = (num) => {
  return num > maxNum;
}
...
```js title="순수함수(Pure Function)" subtitle="Immutable"
...
```js title="순수함수(Pure Function)" subtitle="기존 값을 변경하지 않고, 새로운 값을 반환. 즉, 사이드 이펙트 없음."
const replaceSpace = (str) => {
  return str.replace(/(_|-)/, ' ');
}
...
그에 반해, 이 negate 함수는 함수 밖에 있는 변수 값에 의존하고 있습니다. 이런 함수를 불순 함수라고 부릅니다. 이런 경우 num 값이 다른곳에서 바뀌게 되면 함수의 결과가 매번 달리지게 되겠죠. 이런 예제를 하나 들면 [enter]
...
이렇게 20으로 바뀌게 되면 같은 숫자인 10을 넘겨도 결과는 달라지게 됩니다. 이런 부분들이 의도치 않은 사이드이펙트를 발생시키는 부분이겠죠. [enter]

그리고 순수함수는 파라미터로 받은 값도 변경하지 않습니다. 이런 특성을 Immutable이라고 합니다.
...
<Notes>
다시 한번 정리하면 함수형 프로그래밍은 일급함수와 순수함수 라는 특징을 갖습니다.
</Notes>
...
함수형 프로그래밍은 뭔가 대단한 개념이 아니라 작은 함수들을 조합해서 원하는 목적을 만들어낸다고 생각하시면 되는데요.

이런 합수를 조합하는 행위를 함수 합성이라고 부릅니다.
...
```js file=./src/FunctionComposition/1-1.js subtitle="여러 헬퍼 함수들"
```
...
여기 그냥 몇가지 함수를 정의했습니다. 제곱을 하는 함수, 음수로 만드는 함수, 1을 더하는 함수가 있습니다. [enter]
...
그런데 이런 형태로 적용해야 하는 함수가 더 많아진다고 하면 어떨까요? 읽기가 더욱 힘들어 집니다.  [enter]
...
<Step subtitle="읽기 편하게 바꿔줄 compose 함수">
...
자 그래서 이런식으로 함수를 조합했을 때 읽기 어려운 부분을 조금더 읽기 쉽게 도와줄 함수를 하나 만들어 사용해보겠습니다.
...
그런데 compose 합성을 한 이유가 조금 더 읽기 쉽게 만들기 위함이었는데 뭔가 조금 더 읽기 어려워 진것 같슴니다.

이 부분을 조금 더 개선하기 위해서 포인트프리 스타일을 살펴보도록 하겠습니다.
...
포인트프리 스타일은 방금전 살펴봤던 약간은 복잡해 보이는 코드를 조금 더 읽기 쉽게 바꿔줄 수 있는 방법입니다.
...
처음 구현했던 코드와 거슬리는 부분을 제거한 코드입니다.

여기 두 코드의 큰 차이점은 파라미터 정보를 숨겼다는 점인데요. [enter]

이런 스타일의 코드를 바로 포인트프리 스타일이라고 부릅니다. 이런 단어 자체가 중요한게 이나리 이런 방식으로 사용할 수 있다는 점만 알고 넘어가시면 될것 같습니다.

포인트프리 스타일 코드는 불필요한 파라미터 정보를 제거할 수 있어서 복잡해 보일 수 있는 코드를 간단하게 만들 수 있습니다.
...
그래서 이 순서를 왼쪽에서 오른쪽으로 실행하도록 바꾸고 싶은데요. [enter]

그러기 위해서 compose 구현을 조금 바꿔주도록 하겠습니다. [enter]

함수 실행이 역순으로 되는 이유는 이부분에 있는 reduceRight 때문입니다. 그래서 이부분을 reduce로 바꿔주면 됩니다. [enter]
...
방향은 사실 개인 취향에 따라 다른거기 때문에 아무거나 사용해도 되겠지만

저는 개인적으로 pipe 방식이 더 편하기 때문에 이 이후로 나오는 예제에서는 모두 pipe를 사용해서 함수 합성을 진행하도록 하겠습니다.
...
저는 이 커링이라는 개념을 처음 알았을때 왜 굳이 이렇게 파라미터를 쪼개서 받는걸까 하고 이해를 못했습니다.

뭐 임시로 일부만 바인딩해서 나중에 쓴다고 쳐도 크게 메리트가 없어보였습니다.

그런데 이 커링이 크게 도움이 되는 부분을 함수형 프로그래밍을 하면서 발견했는데요 한번 살펴보도록 하겠습니다.
...
그런데 여기서는 아까 알아봤던 포인트프리 스타일을 사용하지 않았습니다. 왜그럴까요?

왜냐하면 여기 익명함수에 받는 파라미터 수와 dissoc, rename 함수가 받는 파라미터 수가 다르기 때문입니다.

그런데 [enter] dissoc과 rename의 파라미터를 하나로 만들수만 있다면 포인트프리 스타일을 사용할 수 있을것 같습니다.

그럼 이 파라미터 두개짜리 함수를 어떻게 파라미터 하나짜리 함수로 만들 수 있을까요? 여기서 등장하는 것이 바로 커링입니다. [enter]
...
자 그럼 이 커링이 어떻게 포인트프리 스타일에 도움이 되는지 다시 한번 살펴보겠습니다.
...
그런데 이 코드처럼 첫번째 파라미터인 'age'만 먼저 넘겨주고, [enter]

이후에 나머지 파라미터를 받아서 실행할수 있다면 'age'만 미리 바인딩해서 파라미터 하나를 받는 함수로 만들어 줄 수 있을것 같습니다.

그러면 파라미터 수가 맞아서 포인트프리 스타일로 사용할 수 있습니다. [enter]

이걸 구현하기 위해서 dissoc의 구현을 조금 바꿔줍니다. [enter]

먼저 파라미터를 하나만 받는 함수로 바꿔주구요. 이 함수의 리턴 값으로 나머지 파라미터를 받는 함수를 반환해줍니다. [enter]

rename도 같은 방식으로 바꿔줍니다. [enter] [enter]
...
그럼 이렇게 바꿔준 함수를 가지고 다시 한번 구현해 보겠습니다. [enter]

dissoc에 age를 먼저 넘겨주면 person만 받는 새로운 함수가 반환됩니다. [enter]

그리고 rename도 같은 방식으로 사용하면 우리가 원했던 포인트 프리 스타일로 사용할 수 있습니다. [enter]

이렇게 원래 2개의 파라미터를 받는 함수를 하나씩 잘라서 파라미터를 받을수 있도록 처리하는 기법을 커링이라고 합니다. 

그런데 지금은 이렇게 파라미터가 2개라서 직접 커링으로 구현하기가 쉬웠지만 매번 모든 함수를 이렇게 커링하거나 파라미터 많은 함수를 직접 커링하기는 너무 힘들기 때문에

자동으로 커링을해줄 헬퍼 함수를 만들어 줍니다.
</Notes>
...
이 함수를 이용하면 dissoc과 rename을 직접 커링 해주던 부분을 자동으로 버꿔서 사용할 수 있습니다.

자 이렇게 커링을 사용하면 파라미터를 하나로 만들어줄 수 있기 때문에 함수합성시에 포인트프리 스타일로 코드를 작성할 수 있어서

좀 더 깔끔한 코드를 만들 수 있습니다.
...
다음으로 함수형 프로그래밍 스타일 비교에 대한 내용인데요.

함수형 프로그래밍에서 함수를 합성할 때 두가지 방식이 있습니다.

하나는 메소드 체이닝이고 또 하나는 파이프라이닝입니다.

지금까지 살펴봤던 pipe나 compose를 사용한 방식이 파이프라이닝 방식이고, 메소드 체이닝 방식은 맨 처음 살펴봤던 예제에서 나왔었는데요.

이 두가지 스타일을 비교해보고 어떤 부분에서 차이가 있는지 살펴보겠습니다.
...
메소드 체이닝은 lodash라는 라이브러리르 가지고 진행을 할건데요.

로대시는 간단하면서도 유용한 여러 유틸 함수를 제공합니다.

뿐만 아니라 LodashWrapper라는 객체를 이용해서 체이닝을 제공하기 때문에 이 라이브러리를 가지고 설명을 하겠습니다.

lodash는 chain이라는 함수를 이용해서 메소드 체이닝을 시작합니다.

그리고 점(.)을 이용해 체이닝을 이용해 데이터를 처리하는데요.

마지막에 value를 호출해서 최종적으로 값을 계산하도록 합니다.

_.chain에 names을 파라미터로 호출 하면 내부적으로 names를 들고있는 LodashWrapper 라는 객체가 반환됩니다. [enter]
...
파이프라이닝은 lodash의 함수형 프로그래밍 모듈인 lodash/fp를 사용했습니다.

lodash/fp에 있는 모든 함수는 기본적으로 커링이 돼있기 때문에 포인트 프리 스타일을 사용할 수 있습니다. [enter]
...
age 라는 프로퍼티를 지우기 위해서 LodashWrapper에 있는 omit과 lodash/fp의 omit을 사용합니다. 

work라는 프로퍼티를 job으로 바꿔줘야 하는데요. [enter]

lodash에서는 프로피터 명을 바꿔주는 메소드를 제공하지 않습니다. 그래서 직접 구현해서 사용해야 하는데요.

메소드 체이닝 방식은 직접 군현했다고 하더라도 LodashWrapper에서 제공해주는 메소드가 아니기때문에 체이닝을 붙일수가 없습니다.

하지만 파이프라이닝은 그냥 함수를 중간에 끼워 넣기만 하면 되기 때문에 이런식으로 다음 함수로 끼워줄 수 있습니다.  [enter]

결국 메소드 체이닝 방식은 age를 삭제한 결과를 중간에 저장하고, 마지막헤 rename 함수를 이용해 다시한번 요구사항을 만족시켜야 합니다. [enter]

그래서 이런 부분에서 단단한 결합과 유연함의 차이가 생기게 되구요.

결과적으로 파이프라이닝이 어떤 로직을 구현하는데 있어서 조금더 유연하게 구현할수 있다 라는게 큰 장점으로 올 수 있습니다.
...
그리고 이 상자 안에 값을 저장할수 있게 조금 바꿔보겠습니다. [enter]
...
자, 우선 책을 찾아야 하기 때문에 책 아이디와 책 목록을 받아서 책을 찾아주는 findBookById를 만들어 줍니다. [enter]

그리고 찾은 책에서 title 값을 가져오기 위해 prop 함수를 만들어 줍니다. [enter]
객체에서 특정 프로퍼티 이름에 해당하는 값을 반환해주는 함수입니다. [enter]

마지막으로 제목의 첫 글자를 대문자로 바꿔줘야 하기 때문에 startCase라는 함수를 만들어 줍니다. [enter]

그럼 이제 최종 결과를 만들어줄 함수를 작성해보도록 하겠습니다. 이 함수는 id와 책을 파라미터로 받아서 처리 합니다. [enter]

pipe함수를 가지고 박스 안에 books를 넣어 줍니다.[enter]

다음으로 우리가 만들었던 책을 찾는 함수인 findBookById를 사용하는데요. 책 목록이 박스 안에 들어있기 때문에 box.$value를 사용해서 책을 찾아 줍니다. [enter]
...
상자에 값을 넣었다가 다시 $value로 빼서 사용할꺼면 애초에 넣을필요가 없지 않나 하는 생각이 듭니다.

그래서 적어도 상자에 값을 넣은 행동에 의미가 있도록 상자를 유지한 상태로 값을 바꿔보도록 하겠습니다. [enter]
...
이런 식으로 map 메소드를 이용하면 박스를 유지한 채로 값을 변경할 수 있게 됐습니다.

게다가 반환 값이 박스 인스턴스이므로 반환 값에 다시 map을 이용해서 값을 변경할 수 있습니다. 메소드 체이닝 방식이죠.

그런데 사실은 박스 안에 있는 값을 변경했다기 보다 map 메소드를 이용해서 새로운 박스 인스턴스를 반환했다고 말하는게 더 정확합니다.

이전 박스 인스턴스의 값을 변경하지 않고 새로운 Box 인스턴스를 만들었으니까 그런거겠죠. 바로 이부분이 함수형 프로그래밍의 특징중 하나인 순수함수의 immutable이 되겠습니다. [enter]
...
여기부터는 박스에 값을 넣었기 때문에 박스 형태를 유지하기 위해서 map 함수를 사용해야 합니다. map 함수에 findBookById를 사용하면 박스 안에 책을 넣은 형태로 반환이 됩니다. [enter]
...
함수형 프로그래밍에서 함수 합성을 할 때 문제가 되는 부분이 있는데요.

바로 예외처리가 힘들다는 점입니다. 상황을 한번 살펴보겠습니다.
...
안타 깝게도 이런 에러가 발생하면서 중간에 프로그램이 멈추겠죠. 왜일까요?[enter]
...
여기서 에러가 발생하게 됩니다. 그럼 prop 함수에서 에러가 발생했으니 prop 함수 내에서 예외처리를 해주면 될것 같습니다. [enter]
...
그래서 이 문제를 우아하게 해결할 방법이 펑터를 이용한 방법입니다. 이 Maybe도 펑터의 한 종류인데요. 아까 살펴봤던 박스보다도 조금 더 특별한 기능을 갖는 펑터입니다.
...
<Step title="Maybe 펑터" subtitle="두 가지 상태">
...
이 메이비라는 펑터는 내부적으로 두 가지 상태값을 갖습니다.
...
두 가지 상태는 박스 안에 들어있는 값에 따라 결정이 되는데요. 박스 안의 값이 undefined이거나 null인 경우 Nothing 상태가 되고. 그 외에는 Just 상태가 됩니다.
...
그리고 조금 특별한 기능이 바로 이부분인데요. 상태에 따라 map의 구현이 살짝 달라집니다. 

Maybe가 Nothing 상태일 경우는 파라미터 받은 함수는 무시하고 Nothing 상태인 자기 자신을 그대로 반환해 줍니다.
그리고 Just 상태인 경우는 기존 박스와 마찬가지로 함수를 실행해서 다시 Maybe에 감싸서 반환해 줍니다. [enter]

자 그럼 이전에 구현했던 부분중에 Box를 Maybe로 바꿔보겠습니다.[enter]
...
More 👉 https://nakta.dev/fp-in-js-1-code-style

<Notes>

사실 오늘 더 많은 내용을 하고 싶었는데 그러다가는 한시간을 홀쩍 넘길것 같아서 여기까지 했구요.

혹시나 오늘 발표한것 외에 더 알고 싶다 하시는 분들은 인터넷 찾아보시면 좋은 글들이 많으니 찾아보셔도 되구요.

에전에 제가 정리했던 블로그 글이 있습니다. 이걸 참고하셔도 좋을것 같습니다.
