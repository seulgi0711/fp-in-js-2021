...
그리고 완벽하게 반대라고 하기는 그렇지만 함수형 프로그래밍과 같이 자주 비교되는 '절차지향형 프로그래밍'같은 단어들도 있습니다.
...
여기 이름 목록이 있습니다. 그런데 입력한 형태가 통일성이 없습니다.
...
첫 번째로 성과 이름은 공백으로 구분하려고 합니다.

언더바와 대시 문자를 공백으로 맞춰주는 작업을 할 거구요. [enter]
...
함수형 프로그래밍의 특징 중 하나는 일급 함수를 사용한다는 점인데요.

일급 함수라는 뜻은 함수를 변수와 같이 사용할 수 있다는 뜻입니다.
...
함수형 프로그래밍은 뭔가 대단한 개념이 아니라 단순히 작은 함수들을 조합해서 원하는 목적을 만들어낸다고 생각하시면 되는데요.

이런 합수를 조합하는 행동을 함수 합성이라고 부르고 함수합성 방법에 대해서 살펴보도록 하겠습니다.
...
그런데 이런 형태로 적용해야 하는 함수가 더 많아진다고 하면 어떨까요? 읽기가 더욱 힘들어 집니다.

그리고 이런 형태는 우리 모두가 어렸을적 수학시간에 배웠던 수학적인 함수 합성 표현법과 같습니다. [enter]
...
여기 왼족에서 오른쪽에 해당하는 부분이 지금 오른쪽 코드와 같은 형태를 띄고 있습니다.

안쪽부터 적용을 해 나가는 모습입니다.
...
이런 수학적 표현 방식을 읽기 편하게 바꿔줄 수 있는 함수를 하나 만들어 사용해보겠습니다.
...
자 이렇게 해서 방금, 이번 주제인 펑터를 만나게 됐습니다.
...
Maybe의 Nothing 상태일 경우는 파라미터 받은 fn무시하고 Nothing 상태인 자기 자신을 그대로 반환해 줍니다.
